...
const CreateSurface = async (colormapName: string, meshColor = [1,1,1], isAnimation = true, dy= 0) => {
    const data = SimpleSurfaceData(Sinc, -8, 8, -8, 8, 30, 30, 2, 0.3, colormapName);
    const mesh = SimpleSurfaceMesh(Sinc, -8, 8, -8, 8, 30, 30, 2, 0.3,[0,0,0], dy);
    const mesh2 = SimpleSurfaceMesh(Sinc, -8, 8, -8, 8, 30, 30, 2, 0.3,[0,0,0], -dy)
